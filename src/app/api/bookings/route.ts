...
export async function GET() {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: data.error || 'Failed to fetch bookings' }, { status: 400 })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: data.error || 'Failed to delete booking' }, { status: 400 })
    } catch (fetchError: unknown) {
      clearTimeout(timeoutId)
